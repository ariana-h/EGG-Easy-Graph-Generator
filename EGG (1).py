        ...
        try:
            ...
            if file_path.endswith('.csv'):
                ...
                with open(file_path, newline='') as csvfile:
                    ...
                    for row in reader:
                        # Each row should be a single string in quotes or two values separated by comma
                        if len(row) == 1 and ',' in row[0]:
                            ...
                            messagebox.showerror("Error", "Data is not in expected 'x,y' format.")
                            return

            elif file_path.endswith('.xlsx'):
                # Load excel data
                data = pd.read_excel(file_path, header=None, names=["x", "y"], engine='openpyxl')
            ...
            messagebox.showerror("Error", "Failed to import and plot  data: {e}")
